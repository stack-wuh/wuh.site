...
const useScroll = () => {
  const scroll = (direction = 1) => {
    window.scroll(0,
      window.innerHeight + direction * (document?.documentElement?.scrollTop ?? document?.body?.scrollTop ?? 0)
    )
  }
  ...
  /**回到页尾 */
  const scrollToBottom = (): void => window.scroll(0, getTargetRects().scrollTop + getTargetRects().innerHeight)
